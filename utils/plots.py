...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    # img[y:h, X:w] = cv2.addWeighted(sub_img, 0.8, img2,1- 0.8, 1)
    

    # img3=cv2.rectangle(img, c1, c2, (0, 0, 151), cv2.FILLED)
    # img2 = img.copy()
    # cv2.rectangle(img2, c1, c2, (0, 0, 200), -1)
    # alpha = 0.4
    # img = cv2.addWeighted(img, alpha, img2, 1-alpha, 0)
    # cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    # img[y:w, X:h] = img[y:w, X:h] * 0.5 + \
    ...
    if label:
        ...
        X, y = int(x[0]), int(x[1]) - t_size[1]-3
        w = c1[0] + t_size[0]
        h = y + t_size[1]+3
        alpha = 0.5 # opecity
        img[y:h, X:w] = img[y:h, X:w] *0.5 + \
            np.array(color, dtype=np.uint8) * 1-0.5
        # sub_img = img[y:h, X:w]
        # img2 = np.full(sub_img.shape, color, np.uint8)
        # img[y:h, X:w] = cv2.addWeighted(sub_img, 0.8, img2,1- 0.8, 1)
